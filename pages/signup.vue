    ...
    <NuxtLayout name="auth">
      <div class="bg-white p-8 rounded-lg shadow-md">
        <div class="text-center mb-8">
          ...
          <p class="text-gray-600 mt-2">Create an account</p>
        </div>
        ...
        <div class="mt-6 text-center">
          <p class="text-sm text-gray-600">
            ...
            <NuxtLink to="/signin" class="text-blue-600 hover:text-blue-700">
              Sign in
            </NuxtLink>
          </p>
        </div>
...
const handleSignUp = async () => {
  ...
  try {
    if (form.password.length < 8) {
      throw new Error('Passwords minimum length is 8 characters')
    }
    ...
    notification.value = error instanceof Error ? error.message : 'An error occurred during registration' 
  } finally {
    loading.value = false
  }
}
